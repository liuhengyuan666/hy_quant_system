...
from core.trading_calendar import is_preclose_window, is_trading_session, latest_closed_trading_date, resolve_preclose_signal_date
...
class TradingCalendarTests(unittest.TestCase):
    ...
    def test_preclose_window_detection(self):
        runtime = RuntimeConfig(
            timezone="Asia/Shanghai",
            intraday_enabled=True,
            intraday_interval_minutes=5,
        ...
        self.assertFalse(is_preclose_window(datetime(2026, 3, 10, 14, 40, tzinfo=ZoneInfo("Asia/Shanghai")), runtime))
        self.assertTrue(is_preclose_window(datetime(2026, 3, 10, 14, 46, tzinfo=ZoneInfo("Asia/Shanghai")), runtime))

    def test_resolve_preclose_signal_date_uses_same_day_for_intraday_snapshot(self):
        runtime = RuntimeConfig(
            timezone="Asia/Shanghai",
            intraday_enabled=True,
            intraday_interval_minutes=5,
        ...
        reference = datetime(2026, 3, 10, 14, 50, tzinfo=ZoneInfo("Asia/Shanghai"))
        self.assertEqual(str(resolve_preclose_signal_date(reference, runtime, use_intraday_snapshot=True)), "2026-03-10")
        self.assertEqual(str(resolve_preclose_signal_date(reference, runtime, use_intraday_snapshot=False)), "2026-03-09")
